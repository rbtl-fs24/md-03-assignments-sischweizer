...
```{r}
penguins |> filter(sex == "male" & island %in% c("Dream", "Biscoe")) 
```
...
```{r}
penguins |> filter(sex == "female" & species == "Adelie" & bill_length_mm < 35)
```
...
```{r}
penguins |> filter(sex == "female" & species == "Chinstrap" & island %in% c("Dream", "Torgersen"))
```
...
```{r}
penguins |> filter(species == "Gentoo" | body_mass_g > 4500)
```
...
```{r}
penguins |> mutate(flipper_m = flipper_length_mm / 1000)
```
...
```{r}
penguins |> mutate(id = seq(1:n())) |> relocate(id) 
```
