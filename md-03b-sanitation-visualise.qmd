...
library(ggthemes)
library(usethis)
use_git_config(user.name = "Simon Schweizer", user.email = "sischweizer@student.ethz.ch")
```
...
```{r}
sanitation_afg <- sanitation |> filter(iso3 == "AFG" & year %in% c(2000, 2020) 
...
2.  What would you want to change?
    -   the order of the bar stacking
3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?
    -   to be honest, i dont know. There are three more objects when using the method count but the plot didn't change when i removed the filter argument of "sm".

## Task 7

1.  Run the code in the code chunk below.
    1.  What do you observe when you look at the code and plot?
        -   san_sm is the only variable which is widely spread, all others have a clear pattern.
